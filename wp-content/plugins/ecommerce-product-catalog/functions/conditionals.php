...
function is_ic_product_listing( $query = null ) {
	...
	if ( ! isset( $this_query->is_ic_product_listing ) ) {
		...
		} else if ( ! empty( $query ) && ( ( $query->is_post_type_archive( product_post_type_array() ) && ! $query->is_search() && ! is_ic_taxonomy_page( $query ) && ! is_ic_product_search( $query ) ) || is_home_archive( $query ) || is_custom_product_listing_page( $query ) ) ) {
			$this_query->is_ic_product_listing = 1;
		} else {
...
function ic_is_wp_query_available() {
	global $wp_query;
	if ( empty( $wp_query->query ) && ! ic_is_wp_object_available() ) {
