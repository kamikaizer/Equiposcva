	...
	function price_td( $products_table, $cart_id, $quantity, $settings, $raw, $product_price ) {
		...
		if ( $product_price !== '' && $product_price !== false ) {
			...
			if ( isset( $settings['cart_page_template'] ) && $settings['cart_page_template'] == 'qty' ) {
				...
				$products_table .= price_format( $price, 1, 0 );
				$products_table .= '</td>';
			}
			$products_table .= apply_filters( 'ic_cart_checkout_additional_columns', '', $product_id, $cart_id, $quantity, $raw );
			if ( $raw ) {
				$products_table .= '<td class="td-total"><input type="hidden" hidden name="product_price_' . sanitize_title( $cart_id ) . '" value="' . price_format( $price, 1, 0 ) . '">' . price_format( $p_total, 1, 0 ) . '</td>';
			} else {
				$products_table .= '<td class="' . $product_id . '_total product_total td-total">' . price_format( $p_total, 1, 0 );
