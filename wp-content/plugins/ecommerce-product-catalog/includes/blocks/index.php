...
class ic_epc_blocks {
	...
	function __construct() {
		if ( function_exists( 'register_block_type' ) ) {
			...
			add_action( 'current_screen', array( $this, 'register_scripts' ) );
...
<!-- /wp:shortcode -->';
	...
	function enqueue() {
		...
		wp_localize_script( 'ic-epc-show-catalog', 'ic_epc_blocks', apply_filters( 'ic_epc_blocks_localize', array(
				'strings'                  => array(
	...
	function register_scripts() {
		$deps   = array( 'wp-blocks', 'wp-element', 'wp-i18n', 'ic_chosen' );
		$screen = get_current_screen();
		if ( $screen->id === 'widgets' ) {
			$deps[] = 'wp-edit-widgets';
		} else if ( $screen->id === 'customize' ) {
			$deps[] = 'wp-customize-widgets';
		} else {
			$deps[] = 'wp-editor';
		}
		wp_register_script( 'ic-epc-show-catalog', AL_PLUGIN_BASE_PATH . 'includes/blocks/js/show-catalog-block.min.js' . ic_filemtime( AL_BASE_PATH . '/includes/blocks/js/show-catalog-block.min.js' ), $deps, null, true );
		wp_register_script( 'ic-epc-show-products', AL_PLUGIN_BASE_PATH . 'includes/blocks/js/show-products-block.min.js' . ic_filemtime( AL_BASE_PATH . '/includes/blocks/js/show-products-block.min.js' ), $deps, null, true );
		wp_register_script( 'ic-epc-show-categories', AL_PLUGIN_BASE_PATH . 'includes/blocks/js/show-categories-block.min.js' . ic_filemtime( AL_BASE_PATH . '/includes/blocks/js/show-categories-block.min.js' ), $deps, null, true );
		do_action( 'ic_register_block_scripts', $deps );
	}
